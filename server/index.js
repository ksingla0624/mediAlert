...
// database connection
const URL = "mongodb://localhost:27017/authMern";
connection();
...
cron.schedule("*/2 * * * *", async function () {
  ...
  for (let j = 0; j < stock.length; j++) {
    ...
    let msg =
      "Your stock for medicice " +
      stock[j].medicineName +
      "is Below 10, Please fill this up";
    console.log(msg);
  ...
  for (let i = 0; i < medicineTime.length; i++) {
    if (
      Number(medicineTime[i].medicineTime.split(":")[0]) === date.getHours() &&
      Number(medicineTime[i].medicineTime.split(":")[1]) - date.getMinutes() <=
        5 &&
      Number(medicineTime[i].medicineTime.split(":")[1]) - date.getMinutes() >=
        0
    ) {
      ...
      var mailOptions = {
        ...
        html:
          "<p>Hey Dear, It's time to take the medicine <b><i>" +
          medicineTime[i].medicineName +
          " .</i></b></p><br><br><p>Regards,</p><h3>MyMeds<br>We Care For Your Health!</h3>",
      };
